...
extension MessageSender {
    ...
    private class func ensureSessions(forMessageSend messageSend: OWSMessageSend,
        ...
        for deviceId in deviceIdsWithoutSessions {
            ...
            let promise: Promise<Void> = firstly(on: .global()) { () -> Promise<AppServiceKit.PreKeyBundle> in
                let (promise, resolver) = Promise<AppServiceKit.PreKeyBundle>.pending()
                ...
                return promise
            }.done(on: .global()) { (preKeyBundle: AppServiceKit.PreKeyBundle) -> Void in
...
public extension MessageSender {
    ...
    class func makePrekeyRequest(messageSend: OWSMessageSend,
                                 deviceId: NSNumber,
                                 accountId: AccountId?,
                                 success: @escaping (AppServiceKit.PreKeyBundle?) -> Void,
        ...
        firstly(on: .global()) { () -> Promise<RequestMakerResult> in
            ...
            let bundle = AppServiceKit.PreKeyBundle(from: responseObject, forDeviceNumber: deviceId)
            success(bundle)
    ...
    class func createSession(forPreKeyBundle preKeyBundle: AppServiceKit.PreKeyBundle,
                             accountId: String,
    ...
    class func handleUntrustedIdentityKeyError(accountId: String,
                                               recipientAddress: SignalServiceAddress,
                                               preKeyBundle: AppServiceKit.PreKeyBundle,
