...
@objc
public class WLTNavigationBar: UINavigationBar {
    ...
    public var statusBarHeight: CGFloat {
        ...
        return 44//CurrentAppContext().statusBarHeight
    }
...
//        }
    ...
    private func applyTheme() {
        ...
        if [.secondaryBar, .solid].contains(currentStyle) {
            ...
            if let tintingView = blurEffectView.subviews.first(where: {
                ...
                tintingView.backgroundColor = navbarBackgroundColor.withAlphaComponent(WLTNavigationBar.backgroundBlurMutingFactor)
                self.setBackgroundImage(UIImage(), for: .default)
            } else {
                if #available(iOS 15, *) { fatalError("Check if this still works on new iOS version.") }
                
                fatalError("Unexpectedly missing visual effect subview")
    ...
    public func switchToStyle(_ style: NavigationBarStyle, animated: Bool = false) {
        ...
        if animated {
            ...
            animation.type = .fade
            layer.add(animation, forKey: "wlt_fade")
        } else {
            layer.removeAnimation(forKey: "wlt_fade")
        }
