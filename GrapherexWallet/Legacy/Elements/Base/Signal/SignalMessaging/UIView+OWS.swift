...
public extension UIView {
    ...
    class func hStretchingSpacer() -> UIView {
        let view = UIView()
        view.wltSetContentHuggingHorizontalLow()
        view.wltSetCompressionResistanceHorizontalLow()
        return view
    }
    ...
    class func vStretchingSpacer(minHeight: CGFloat? = nil, maxHeight: CGFloat? = nil) -> UIView {
        let view = UIView()
        view.wltSetContentHuggingVerticalLow()
        view.wltSetCompressionResistanceVerticalLow()
    ...
    func applyScaleAspectFitLayout(subview: UIView, aspectRatio: CGFloat) -> [NSLayoutConstraint] {
        ...
        constraints.append(subview.wltAutoPin(toAspectRatio: aspectRatio))
    ...
    func animateDecelerationToVerticalEdge(
        ...
        if velocity != .zero {
            ...
            if velocity.x > 0 {
                timeUntilVerticalEdge = ((boundingRect.maxX - wltWidth) - currentPosition.x) / velocity.x
            ...
            if velocity.y > 0 {
                timeUntilHorizontalEdge = ((boundingRect.maxY - wltHeight) - currentPosition.y) / velocity.y
            ...
            if timeUntilHorizontalEdge > timeUntilVerticalEdge {
                intersectPoint = CGPoint(
                    x: velocity.x > 0 ? (boundingRect.maxX - wltWidth) : boundingRect.minX,
                    y: (timeUntilVerticalEdge * velocity.y) + currentPosition.y
                )
            } else {
                intersectPoint = CGPoint(
                    x: (timeUntilHorizontalEdge * velocity.x) + currentPosition.x,
                    y: velocity.y > 0 ? (boundingRect.maxY - wltHeight) : boundingRect.minY
                )
            }
...
public extension CGPoint {
    ...
    func plus(_ value: CGPoint) -> CGPoint {
        return WLTCGPointAdd(self, value)
    }

    func plusX(_ value: CGFloat) -> CGPoint {
        WLTCGPointAdd(self, CGPoint(x: value, y: 0))
    }

    func plusY(_ value: CGFloat) -> CGPoint {
        WLTCGPointAdd(self, CGPoint(x: 0, y: value))
    }

    func minus(_ value: CGPoint) -> CGPoint {
        return WLTCGPointSubtract(self, value)
    }
...
public extension UITextView {
    ...
    func updateVerticalInsetsForDynamicBodyType(defaultInsets: CGFloat) {
        let currentFontSize = UIFont.wlt_dynamicTypeBody.pointSize
