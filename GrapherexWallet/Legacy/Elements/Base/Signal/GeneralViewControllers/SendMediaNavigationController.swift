...
@objc
class SendMediaNavigationController: WLTNavigationController {
    static var bottomButtonsCenterOffset: CGFloat {
        if UIDevice.current.hasIPhoneXNotch {
            ...
            return 0//-1 * (CaptureButton.recordingDiameter / 2 + 4) - fixedBottomSafeAreaInset
        } else {
            return 0//-1 * (CaptureButton.recordingDiameter / 2 + 4)
        }
    }
    ...
    var attachmentCount: Int {
        return 0//attachmentDraftCollection.count
    }
...
//        }
    ...
    override func viewDidLoad() {
        ...
        batchModeButton.wltSetCompressionResistanceHigh()
        
        view.addSubview(doneButton)
        doneButton.wltSetCompressionResistanceHigh()
        
        view.addSubview(cameraModeButton)
        cameraModeButton.wltSetCompressionResistanceHigh()
        
        view.addSubview(mediaLibraryModeButton)
        mediaLibraryModeButton.wltSetCompressionResistanceHigh()
...
//            }
    ...
    private func didTapCameraModeButton() {
        self.wlt_askForCameraPermissions { isGranted in
...
//            self.fadeTo(viewControllers: [self.captureViewController], duration: 0.08)
    ...
    private func didTapMediaLibraryModeButton() {
        self.wlt_askForMediaLibraryPermissions { isGranted in
            guard isGranted else { return }
...
//        updateViewState(topViewController: viewController, animated: false)
    ...
    private func updateNavbarTheme(for viewController: UIViewController, animated: Bool) {
        let showNavbar: (WLTNavigationBar.NavigationBarStyle) -> Void = { navigationBarStyle in
            if self.isNavigationBarHidden {
                self.setNavigationBarHidden(false, animated: animated)
            }
            guard let owsNavBar = self.navigationBar as? WLTNavigationBar else {
...
//        let container = PillView()
        ...
        container.backgroundColor = .wlt_white
        container.layoutMargins = UIEdgeInsets(top: 7, left: 8, bottom: 7, right: 8)
...
//        let badge = PillView()
        let badge = UIView()
        badge.layoutMargins = UIEdgeInsets(top: 4, left: 4, bottom: 4, right: 4)
        badge.backgroundColor = .wlt_accentBlue
    ...
    private lazy var badgeLabel: UILabel = {
        let label = UILabel()
        label.textColor = .wlt_white
        label.font = UIFont.wlt_dynamicTypeSubheadline.wlt_monospaced
