 ...
 open class TZSegmentedControl: UIControl {
    ...
    public var edgeInset = UIEdgeInsets(top: 0, left: 2, bottom: 0, right: 2)
    ...
    private func setup(){
        let topDivider = UIView.spacer(withHeight: 2)
        topDivider.backgroundColor = Theme.outlineColor
        self.addSubview(topDivider)
        self.addSubview(self.scrollView)
        self.isOpaque = false
        self.contentMode = .redraw
        
        topDivider.autoPinEdge(toSuperviewEdge: .leading)
        topDivider.autoPinEdge(toSuperviewEdge: .trailing)
        topDivider.autoPinEdge(toSuperviewEdge: .bottom)
    }
