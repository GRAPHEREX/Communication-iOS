...
public class QuotedMessageView: ManualStackViewWithLayer {
    ...
    public func configureForRendering(state: State,
        ...
        if configurator.isForPreview {
            stripeView.backgroundColor = conversationStyle.quotedReplyStripeColor(isIncoming: true, isForPreview: true)
        } else {
            stripeView.backgroundColor = conversationStyle.quotedReplyStripeColor(isIncoming: !configurator.isOutgoing, isForPreview: false)
        }
